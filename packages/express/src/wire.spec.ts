...
describe('maayoRouter HTTP boundary', () => {
  it('rejects the reserved system author before persistence over HTTP', async () => {
    ...
    const mutation = makeMutation('01ULID0000000000000000090', 'system');
  ...
  it('acknowledges simultaneous duplicate HTTP requests and preserves unrelated rows', async () => {
    ...
    const baseUrl = await startServer(store);
    const raced = makeMutation('01ULID0000000000000000091');
    const unrelated = makeMutation('01ULID0000000000000000092');
  ...
  it('uses one coherent first-entry outcome for conflicting duplicate IDs', async () => {
    ...
    const baseUrl = await startServer(store, authorizer);
    const deniedId = '01ULID0000000000000000093';
    const reservedId = '01ULID0000000000000000094';
  ...
  it('rejects a timestamp-only continuation over HTTP', async () => {
    const baseUrl = await startServer(new ConcurrentUniqueStore());
  ...
  it('rejects an invalid continuation timestamp over HTTP', async () => {
    const baseUrl = await startServer(new TiedCursorStore());
    const response = await fetch(
      `${baseUrl}/sync/changes?channel=org%3Awire&since=not-a-date&lastMutationId=01ULID0000000000000000101`,
    );
  ...
  it('fails explicitly when a legacy store receives a compound continuation', async () => {
    const baseUrl = await startServer(new ConcurrentUniqueStore());
    const response = await fetch(
      `${baseUrl}/sync/changes?channel=org%3Awire&since=2026-09-02T00%3A00%3A00.000Z&lastMutationId=01ULID0000000000000000101`,
    );
...
async function startServer(store: MaayoStore, authorizer?: ChannelAuthorizer): Promise<string> {
  const app = express();
  app.use(express.json());
  app.use('/sync', maayoRouter({ store, authorizer }));
...
class TiedCursorStore implements MaayoStore {
  readonly ids = [
    '01ULID0000000000000000101',
    '01ULID0000000000000000102',
    '01ULID0000000000000000103',
  ];
